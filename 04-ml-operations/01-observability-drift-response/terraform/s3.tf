...
# bucket é criado e o apply falha do mesmo jeito; nenhum bloco lifecycle, versão de
# provider ou -refresh=false pula essa leitura, porque ela acontece dentro do
# Create.
#
...
resource "terraform_data" "bucket" {
  ...
  # que o nome do bucket viaja no `input` em vez de ser recalculado aqui.
  #
  # O `rb --force` esvazia antes de apagar: o model.tar.gz do treino e os JSON de
  # incidente que a Lambda escreve não são geridos por nenhum aws_s3_object, então
  # um DeleteBucket simples falharia em bucket não vazio. A guarda head-bucket
  # mantém o `make destroy` verde quando o bucket já não existe.
  provisioner "local-exec" {
    when        = destroy
    interpreter = ["/bin/bash", "-c"]
    command     = <<-EOT
...
# Subir pelo Terraform põe os dados no grafo de dependências: o training job não
# consegue começar antes de os bytes que ele lê existirem.
...
# A referência não é canal de treino: ela é a régua contra a qual todo PSI é
# medido. Fica no S3 para a linhagem do que o modelo considera "normal" ser
# auditável depois do lab.
resource "aws_s3_object" "reference" {
  bucket = terraform_data.bucket.output
  key    = "${local.s3_prefixes.reference}/reference.csv"
  source = "${local.data_dir}/reference.csv"
  etag   = filemd5("${local.data_dir}/reference.csv")

  content_type = "text/csv"
}

resource "aws_s3_object" "production_baseline" {
  bucket = terraform_data.bucket.output
  key    = "${local.s3_prefixes.production}/production_baseline.csv"
  source = "${local.data_dir}/production_baseline.csv"
  etag   = filemd5("${local.data_dir}/production_baseline.csv")

  content_type = "text/csv"
}

resource "aws_s3_object" "production_shifted" {
  bucket = terraform_data.bucket.output
  key    = "${local.s3_prefixes.production}/production_shifted.csv"
  source = "${local.data_dir}/production_shifted.csv"
  etag   = filemd5("${local.data_dir}/production_shifted.csv")
