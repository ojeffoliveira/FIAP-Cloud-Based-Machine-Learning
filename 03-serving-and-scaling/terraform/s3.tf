...
resource "terraform_data" "bucket" {
  ...
  #
  # `rb --force` empties before deleting: the training artifact, the async output
  # and the batch transform output are written by SageMaker, not by any
  # aws_s3_object, so a plain DeleteBucket would fail on a non-empty bucket. The
  # head-bucket guard keeps `make destroy` green when the bucket is already gone.
  provisioner "local-exec" {
    when        = destroy
    interpreter = ["/bin/bash", "-c"]
    command     = <<-EOT
